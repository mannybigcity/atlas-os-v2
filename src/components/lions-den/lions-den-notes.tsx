...
export function LionsDenNotesBoard({
  ...
  const visibleNotes = compact ? notes.slice(0, 4) : notes;

  return (
    <section className={compact ? "space-y-4 rounded-[1.2rem] border border-[#d5d0c4] bg-white p-5" : "space-y-5"}>
      {compact ? (
        <div>
          ...
          <h2 className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-[#071b42]">
            {spanish ? "Notas del escritorio" : "Desk notes"}
          </h2>
        </div>
      ) : (
      ...
      {canCreate ? (
        <form action={createOrganizationNote} className={compact ? "space-y-3" : "rounded-[1.6rem] border border-[#d8c27a] bg-white p-5"}>
          ...
          <label className="mt-3 block">
            ...
            <textarea
              className={`mt-2 w-full rounded-xl border border-[#d5d0c4] px-4 py-3 text-sm leading-6 text-[#071b42] ${compact ? "min-h-20" : "min-h-28"}`}
              name="body"
      ...
      {visibleNotes.length === 0 ? (
        <div className={`${compact ? "rounded-xl" : "rounded-[1.6rem]"} border border-dashed border-[#d8c27a] bg-[#fff8e6] p-5 text-sm leading-6 text-[#071b42]`}>
        ...
        <div className="space-y-3">
          {visibleNotes.map((note) => (
            <article className={`${compact ? "rounded-xl" : "rounded-[1.4rem]"} border border-[#d8c27a] bg-white p-4`} key={note.id}>
