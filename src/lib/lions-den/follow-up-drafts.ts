...
import { prospectTelHref } from "./prospect-places.ts";

export const FOLLOW_UP_OWNER_SEND_HINT_EN =
  "Edit the draft, then Email or Text opens your own app with it filled in. Copy works for anything else. Atlas never emails, texts, or calls. When it is out the door, tap “I sent this”.";
export const FOLLOW_UP_OWNER_SEND_HINT_ES =
  "Edita el borrador; Correo o Mensaje abre tu propia app con el texto listo. Copiar sirve para lo demás. Atlas nunca envía correos, mensajes ni llamadas. Cuando lo mandes, toca “Ya lo envié”.";
...
/**
 * Opens the owner's Messages app with the draft filled in. `?&body=` is the
 * form both iOS and Android honor. Texts skip the "Hi name," greeting; that
 * reads stiff in SMS.
 */
export function followUpDraftSms(input: { phone?: string | null; body: string }) {
  const tel = prospectTelHref(input.phone);
  if (!tel) return null;
